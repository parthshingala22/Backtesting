...
// // ── Run card — shows ALL params, user clicks to actually run ──────────────────
...
// // ── Suggestion card — shows ONLY the changed fields (old → new) ───────────────
// // baseForm = the params that were actually used in the last run (not the BacktestForm)
// function SuggestionCard({ changes, baseForm, onConfirm, onDecline, answered, running }) {
//   // changes = only the fields the AI wants to change (e.g. {stop_loss_in_pct: 5})
//   // baseForm = the full params from the last backtest run
...
//   // Per-message card states
//   const [runCardState, setRunCardState] = useState({}); // idx → "running"|"complete"|"declined"
...
//   // lastRunParams: the FULL params used in the most recent real backtest
//   // Used as the baseline for SuggestionCard old-value display
...
//       // Detect card type from user text
//       const isRunIntent = /\b(run|execute|backtest|test this|start|go|launch)\b/i.test(text);
//       const cardType = data.suggested_params
...
//         suggested_params: data.suggested_params || null,  // raw changes only (for suggest) or full params (for run)
//         card_type: cardType,
//         // For suggest card: record the base params at the time of suggestion
...
//   // fullParams must be a COMPLETE form object (all fields)
...
//           role: "assistant",
//           content: data.reply,
//           suggested_params: data.suggested_params || null,  // raw changes only
//           card_type: data.suggested_params ? "suggest" : null,
//           suggestion_base: params, // base = the params just used in this run
//         }]);
//       }
//     } catch { /* non-critical */ }
//   };

//   // ── RunCard: user clicks "Run Backtest" ────────────────────────────────────
...
//   // ── SuggestionCard: user clicks "Run Backtest" ─────────────────────────────
//   // This does NOT run immediately — it inserts a new RunCard with the merged params
//   const handleConfirmSuggestion = (msgIndex, changes, baseForm) => {
//     setSuggestionState(prev => ({ ...prev, [msgIndex]: "confirmed" }));

//     // Merge the AI's changes onto the base params
//     const mergedParams = { ...(baseForm || form || {}), ...changes };

//     // Insert a new RunCard message showing all merged params
//     setMessages(prev => [...prev, {
//       role: "assistant",
//       content: "Here are the optimized parameters ready to run:",
//       suggested_params: mergedParams,   // full merged params for RunCard
//       card_type: "run",
...
//               {/* RunCard — full params, user clicks to run */}
...
//               {/* SuggestionCard — shows only changed fields with old→new diff */}
//               {msg.role === "assistant" && msg.suggested_params && msg.card_type === "suggest" && (
//                 <div className="suggestion-card-wrapper">
//                   <SuggestionCard
//                     changes={msg.suggested_params}           // raw changes only (e.g. {stop_loss_in_pct: 5})
//                     baseForm={msg.suggestion_base}           // full params from last run (for old values)
...
function RunCard({ params, onRun, onDecline, answered, running }) {
  const entries = Object.entries(params).filter(([k, v]) => {
    // Show standard labeled fields
    if (FIELD_LABELS[k]) return true;
    // Also show premium when strike is premium and premium has a value
  ...
  // Custom label for premium field
...
function Chatbot({ isOpen, onClose, backtestResults, form }) {
  ...
  const sendMessage = async (overrideText) => {
    ...
    try {
      ...
      // Detect card type from user text.
      // Informational queries ("tell me about", "describe", "what is") never show a card
      // even if the AI returned suggested_params.
  ...
  const executeBacktest = async (fullParams, runCardMsgIndex) => {
    ...
    try {
      // Build the payload — only remove strike_mode (UI-only field)
      // Keep premium: it is required by the backend when strike_criteria === "premium"
      const payload = { ...fullParams };
      delete payload.strike_mode;  // UI-only, backend doesn't need it
 
      // Ensure premium is a number when present (saved strategies may store it as string)
      if (payload.premium !== undefined && payload.premium !== null) {
        ...
        // Guard: if premium is missing but strike is premium, default to 0
        // This will cause a backend error, but at least it won't crash the frontend
        payload.premium = 0;
      }
  ...
  const handleConfirmSuggestion = (msgIndex, changes, baseForm) => {
    ...
    // If strike changed to ATM, clear premium. If changed to premium and no value, keep base.
